...
execute: 
  echo: false
params:
  dir: "/home/danielred/data/programs/aquaticTS/OUT/test_16"
editor_options: 
...
d9_12 <- function(T, Tmin, Trange, flatness=50, baseline=0.05, pow=3, at=10, constdeath=F){
  if(constdeath){ # use d12
    return( d12m(T=T, Efreeze=flatness, Eheat=pow, death_basel=baseline) )
  } else { # use d9
...
# modifed version: Eheat argoment is negative by definition
d12m <- function(T, Efreeze, Eheat, death_basel, shift=0) {
...
plot_death <- function(Tmin, Trange, Topt, flatness, baseline, pow, range=c(Tmin-Trange/2, Tmin+Trange*1.5 ), constdeath=F){
    ggplot()+
    stat_function(fun= d9_12,
                  geom="area",
                  fill= "lightgreen",
                  args=list(Tmin=Tmin, Trange=Trange, 
                            flatness=flatness, baseline=baseline, pow=pow, 
                            at=Topt, constdeath=constdeath),
                  xlim=c(Tmin,Tmin+Trange),
                  linewidth=3)+
    geom_function(fun= d9_12, 
                  xlim=range, 
                  args=list(Tmin=Tmin, Trange=Trange, 
                            flatness=flatness, baseline=baseline, pow=pow, 
                            at=Topt, constdeath=constdeath))+
...
net_rep <- function(T, R, Tmin, Trange, b, flatness, baseline, pow, A, attack, handling, constdeath=F){
  Tmax= Tmin + Trange
  ...
  d <- d9_12(T, Tmin, Trange, flatness=flatness, baseline=baseline, pow=pow, at=Topt, constdeath = constdeath)
  return(b-d)
}
...
```{r}
if( "use_constant_death" %in% names(par) ){ # new version
  constdeath = as.logical(as.numeric(par$use_constant_death))
  death_variables = par_to_num(par$death)
  # if(constdeath){ # version 2 death
  #   
  # } else { # original death
  #   
  # }
...
death <- ggplot()+
  geom_function(fun = d9_12, 
                xlim=c(minT-10, maxT+10), 
                args=list(Tmin=Tmin, Trange=Trange, 
                          pow=as.numeric(par$death_pow), flatness=as.numeric(par$death_flat), baseline=as.numeric(par$death_basel), 
                          at=Topt, constdeath=constdeath))+
...
plot_grid(plot_grid(birth, death, ncol=2, align="h"), Rn, ncol=1, rel_heights = c(1, 2))
```
