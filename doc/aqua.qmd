...
$$\frac{dT}{dt}= \dots \\
\frac{dN_g}{dt}=N_g r(T) \left( 1-\frac{\sum_{i=1}^{G}N_i}{K} \right) - N_g t_{sleep}(T) + D_g t_{wake}(T)\\
...
## Temperature depedence of $P_{sleep}$

::: {layout="[[3,-0.5,3], [3, -0.5,3]]"}
```{r}
ggplot()+
...
$$P_{sleep} = \frac{1}{1+e^{-T+T_{max}-T_{range}/2 }}$$
...
| $P_{sleep}$ | Rate of dormancy                        | 0.1   |
| $P_{wake}$  | Rate of dormants waking up              | 0.1   |
...
```{r}
outp <- read.table("../output.tsv", header=TRUE)
#str(outp)
