...
func2 <- function(t, state, parameters) {
  with(as.list(c(state, parameters )), {
    list(dX=(amp * sin(omega * t) + mean - (B*X) )/C )
  })
}
parameters = list(amp= 10, mean=20, omega= 2*pi, B= 2, C=1)
out2 <- ode(y = state, times = seq(0, 10, by = 0.01), func = func2, parms = parameters)
...
parameters$mean = T0 * parameters$B
parameters$amp = parameters$B * amplitude / (1 - heat_capacity) 

out2 <- ode(y = state, times = seq(0, 5, by = 0.01), func = func2, parms = parameters)
ggplotly(ggplot(as.data.frame(out2), aes(x=time, y=X))+
  geom_line()+
  labs(x="Time", y="Temperature")+
  theme(legend.position = "none"))
...
let $\hat C = 0.01 \text{ and } C=`r parameters$C`$
...
$$\frac{dR}{dt}= R ~ r_{max} (1 - \frac{R}{K_R(T)}) - f(R) \sum N_g \\
